...
# Color palette
DARK_BG = "#1E1E2E"       # Dark slate
PANEL_BG = "#2A2A40"      # Slightly lighter panel
ACCENT_COLOR = "#4ADE80"  # Soft green accent
TEXT_COLOR = "#F5F5F5"    # Almost white
SECONDARY_TEXT = "#A0A0B0" # Muted gray text
BUTTON_HOVER = "#3FCF6D"  # Lighter green
...
def on_tree_select(event):
    ...
    if index == -1:
        for child in tree.get_children(item_id):
            _, _, cindex = tree.item(child, "values")
            if int(cindex) >= 0:
...
def play_next_in_folder():
    ...
    if cur_pos + 1 < len(siblings):
        _, _, idx2 = tree.item(siblings[cur_pos+1], "values")
        if int(idx2) >= 0:
