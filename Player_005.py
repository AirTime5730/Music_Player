...
custom_frame = None
custom_song_path = None

# --- Custom playback window ---
custom_window_open = False
custom_window_widgets = []

def on_c_pressed():
    global custom_window_open, custom_window_widgets
    if custom_window_open:
        # Close and destroy everything in the custom panel
        for widget in custom_window_widgets:
            widget.destroy()
        custom_window_widgets.clear()
        custom_window_open = False
    else:
        sel_path = find_path_of_current_selection()
        if not sel_path:
        ...
        # Create custom panel in left_frame
        lbl = tk.Label(left_frame, text="Custom Play Settings", fg=TEXT_COLOR, bg=PANEL_BG, font=("Segoe UI", 12, "bold"))
        lbl.pack(pady=(20,5))
        custom_window_widgets.append(lbl)

        # Start time input
        start_lbl = tk.Label(left_frame, text="Start (s):", fg=TEXT_COLOR, bg=PANEL_BG)
        start_lbl.pack()
        start_entry = tk.Entry(left_frame)
        start_entry.pack(pady=(0,5))
        custom_window_widgets.extend([start_lbl, start_entry])

        # End time input
        end_lbl = tk.Label(left_frame, text="End (s):", fg=TEXT_COLOR, bg=PANEL_BG)
        end_lbl.pack()
        end_entry = tk.Entry(left_frame)
        end_entry.pack(pady=(0,5))
        custom_window_widgets.extend([end_lbl, end_entry])

        # Fade time input
        fade_lbl = tk.Label(left_frame, text="Fade (s):", fg=TEXT_COLOR, bg=PANEL_BG)
        fade_lbl.pack()
        fade_entry = tk.Entry(left_frame)
        fade_entry.insert(0, "0")  # default to 0
        fade_entry.pack(pady=(0,10))
        custom_window_widgets.extend([fade_lbl, fade_entry])

        # Play button
        play_btn = ttk.Button(left_frame, text="▶ Play Custom", command=lambda: play_custom(sel_path, start_entry, end_entry, fade_entry))
        play_btn.pack(pady=(0,10))
        custom_window_widgets.append(play_btn)

        custom_window_open = True

def play_custom(path, start_entry, end_entry, fade_entry):
    global paused, pause_cond
    ...
    try:
        start_time_sec = float(start_entry.get())
    except:
        start_time_sec = 0
    try:
        end_time_sec = float(end_entry.get())
    except:
        end_time_sec = get_song_length(path)
    try:
        fade_time_sec = float(fade_entry.get())
    except:
        fade_time_sec = 0

    # Play song from start_time_sec
    pygame.mixer.music.load(path)
    ...
    if fade_time_sec > 0:
        root.after(int((end_time_sec - start_time_sec - fade_time_sec) * 1000),
                   lambda: pygame.mixer.music.fadeout(int(fade_time_sec * 1000)))
    
    # Schedule pause at end_time
    root.after(int((end_time_sec - start_time_sec) * 1000), lambda: pause_custom_play(path))

def pause_custom_play(path):
    global paused, pause_cond
    pygame.mixer.music.pause()
    paused = True
    pause_cond = False
...
root.bind("c", lambda e: on_c_pressed())
